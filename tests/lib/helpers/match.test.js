...
describe('isMatched', () => {
  it('should return true if an entry is matched to a query - without operator', () => {
    ...
    const spyOnIsValidType = jest.spyOn(typesHelper, 'isValidType').mockImplementation(() => true);
    const spyOnIsValidOperator = jest.spyOn(operatorsHelper, 'isValidOperator').mockImplementation(() => false);
    const spyOnIsPassedByOperators = jest.spyOn(operatorsHelper, 'isPassedByOperators').mockImplementation();
    const result = isMatched(query, entry);
    expect(result).toBe(true);
    expect(spyOnIsValidOperator).toHaveBeenCalled();
    expect(spyOnIsValidType).toHaveBeenCalled();
  ...
  it('should return false if an entry is in invalid type - without operator', () => {
    ...
    const spyOnIsValidType = jest.spyOn(typesHelper, 'isValidType').mockImplementation(() => false);
    const spyOnIsValidOperator = jest.spyOn(operatorsHelper, 'isValidOperator').mockImplementation(() => false);
    const spyOnIsPassedByOperators = jest.spyOn(operatorsHelper, 'isPassedByOperators').mockImplementation();
    const result = isMatched(query, entry);
    expect(result).toBe(false);
  ...
  it('should return true if an entry is matched to a query', () => {
    ...
    const spyOnIsValidOperator = jest.spyOn(operatorsHelper, 'isValidOperator').mockImplementation(() => true);
    const spyOnIsPassedByOperators = jest.spyOn(operatorsHelper, 'isPassedByOperators').mockImplementation(() => true);
    const result = isMatched(query, entry);
    expect(result).toBe(true);
    expect(spyOnIsValidOperator).toHaveBeenCalled();
  ...
  it('should return false if an entry is not matched to a query', () => {
    ...
    const spyOnIsValidOperator = jest.spyOn(operatorsHelper, 'isValidOperator').mockImplementation(() => true);
    const spyOnIsPassedByOperators = jest.spyOn(operatorsHelper, 'isPassedByOperators').mockImplementation(() => false);
    const result = isMatched(query, entry);
    expect(result).toBe(false);
    expect(spyOnIsValidOperator).toHaveBeenCalled();
    expect(spyOnIsPassedByOperators).toHaveBeenCalled();
